...
def Attention_Matrix(K, Q, use_mask=False):
	...
	K = tf.transpose(K, perm=[0,2,1]) # 100, 32, 14 

	prod = tf.matmul(Q, K) / np.sqrt(K.shape[0]) # needs to be 100, 14, 14
...
class Atten_Head(tf.keras.layers.Layer):
	def __init__(self, input_size, output_size, use_mask):		
		...
		self.wK = self.add_weight("weight_K",shape=[input_size, output_size])
		self.wQ = self.add_weight("weight_Q",shape=[input_size, output_size])
		self.wV = self.add_weight("weight_V",shape=[input_size, output_size])
	...
	def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
		...
		K = tf.tensordot(inputs_for_keys, self.wK, [[2],[0]])
		V = tf.tensordot(inputs_for_values, self.wV, [[2],[0]])
		Q = tf.tensordot(inputs_for_queries, self.wQ, [[2],[0]])


		atten_matrix = Attention_Matrix(K, Q, self.use_mask)

		res = tf.matmul(atten_matrix, V)
		return res
