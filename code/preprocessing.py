...
def get_report_impressions():
    """
        ...
        Note: will only work/open .txt files
    """
    report_impressions = {}
    for filename in os.scandir("../data/reports"):
        ...
        if (list(tokenized_name[-1])[-3:] != ['t','x','t']): # to aviod open() error -- will only look at .txt's
            continue
...
def make_entries_integers(key_list):
    """
        Input: list of keys as strings
        Output: list of keys turned to integers and then strings again
        Purpose: turn report ids that may have been floats to integers (123456.0 -> 123456)
    """
